...
public class OpencmisDataServiceFacade {
    ...
    public OpencmisDataServiceFacade(OpencmisDataServiceConfig config) {
        ...
        cmisParameters = setupCmisBinding(config);
    ...
    private void addParameter(SessionParameterMap cmisParameters, OpencmisDataServiceConfig config, boolean required, String configParameterName) {
        this.addParameter( cmisParameters, config, required, configParameterName, configParameterName );
    }
    private void addParameter(SessionParameterMap cmisParameters, OpencmisDataServiceConfig config, boolean required, String configParameterName, String cmisParameterName) {
        String parameterValue = config.getValue( configParameterName );
        if (StringUtils.isEmpty( configParameterName ) ) {
            if ( required ) {
                throw new IllegalArgumentException( String.format( "Missing required parameter : '%s'", configParameterName ) );
            } else {
                log.info( "blank parameter : {}", configParameterName );
            }
        } else {
    ...
    private SessionParameterMap setupCmisBinding(OpencmisDataServiceConfig config) {
        SessionParameterMap cmisParameters = new SessionParameterMap();
        ...
        switch (cmisBindingType) {
            case ATOMPUB:
                this.addParameter( cmisParameters, config, true, SessionParameter.ATOMPUB_URL );
                break;
            case BROWSER:
                this.addParameter( cmisParameters, config, true, KEY_BROWSER_URL, SessionParameter.BROWSER_URL );
                break;
            case WEBSERVICES:
                this.addParameter( cmisParameters, config, true, SessionParameter.WEBSERVICES_REPOSITORY_SERVICE );
                this.addParameter( cmisParameters, config, true, SessionParameter.WEBSERVICES_NAVIGATION_SERVICE );
                this.addParameter( cmisParameters, config, true, SessionParameter.WEBSERVICES_OBJECT_SERVICE );
                this.addParameter( cmisParameters, config, true, SessionParameter.WEBSERVICES_VERSIONING_SERVICE );
                this.addParameter( cmisParameters, config, true, SessionParameter.WEBSERVICES_DISCOVERY_SERVICE );
                this.addParameter( cmisParameters, config, true, SessionParameter.WEBSERVICES_RELATIONSHIP_SERVICE );
                this.addParameter( cmisParameters, config, true, SessionParameter.WEBSERVICES_MULTIFILING_SERVICE );
                this.addParameter( cmisParameters, config, true, SessionParameter.WEBSERVICES_POLICY_SERVICE );
                this.addParameter( cmisParameters, config, true, SessionParameter.WEBSERVICES_ACL_SERVICE );
                break;
            default:
                throw new IllegalArgumentException("Invalid " + KEY_BINDING_TYPE);
        }

        this.addParameter( cmisParameters, config, true, KEY_REPOSITORY_NAME, SessionParameter.REPOSITORY_ID );

        String user = config.getValue(KEY_USER);
        String password = config.getValue(KEY_PASSWORD);
        cmisParameters.setBasicAuthentication(user, password);
